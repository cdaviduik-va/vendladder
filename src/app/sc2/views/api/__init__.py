...
from app.sc2.domain.match import close_match, get_suggested_matches, create_match, DEFAULT_TEAM_SIZE
from app.sc2.domain.player import get_all_player_details_for_season, get_players_for_battle_net_names,\
    get_player_stats, get_player_for_email, lookup_players_with_similar_score
...
class MatchResource(BaseAjaxView):
    ...
    def query_suggested(self):
        """ Lookup suggested matches for this season. """
        ignore_open = self.request.get('ignoreOpen') == 'true'
        team_size = int(self.request.get('teamSize', DEFAULT_TEAM_SIZE))
        include_players = self.request.get_all('includePlayers')
        exclude_players = self.request.get_all('excludePlayers')
        match_data = get_suggested_matches(team_size=team_size, include_players=include_players,
                                           exclude_players=exclude_players, ignore_open=ignore_open, limit=5)
        return self.render_response(match_data)

    def query_for_player(self, battle_net_name):
        """ Lookup matches for the given player """
        match_data = MatchModel.lookup_for_battle_net_name(battle_net_name)
        return self.render_response(match_data)
...
class PlayerResource(BaseAjaxView):
    ...
    def query_opponents_for_player(self, battle_net_name):
        players = lookup_players_with_similar_score(battle_net_name)
        return self.render_response(players)
